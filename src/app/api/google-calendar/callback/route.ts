...
export async function GET(req: NextRequest) {
  ...
  const savedState = req.cookies.get("gcal_oauth_state")?.value;
  ...
  try {
    ...
    if (existing) {
      ...
      await supabase.from("data_connectors").insert({
        ...
        description: `Google Calendar for ${userInfo.email || ""}`,
        status: "connected",
        config,
      });
    }
    ...
    await supabase.from("data_sync_log").insert({
      ...
      event_type: "success",
      message: `Connected to Google Calendar for ${userInfo.email || ""}`,
