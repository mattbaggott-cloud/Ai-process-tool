...
import { syncEvents, logActivities, logSync } from "@/lib/google-calendar/sync-service";
...
export async function POST(req: NextRequest) {
  ...
  // Load the Google Calendar connector
  ...
  const steps: SyncStep[] = [
    {
      ...
      fn: (cfg) => syncEvents(cfg, supabase, user.id, orgId, connector.id),
    },
    {
      key: "activities",
      label: "Logging CRM activities",
      fn: () => logActivities(supabase, user.id, orgId),
    },
  ];
      ...
      try {
        ...
        for (let i = 0; i < steps.length; i++) {
          ...
          try {
            ...
            console.error(`Calendar sync step ${step.key} error:`, stepError);
        ...
        await logSync(
          ...
          `Calendar sync completed: ${JSON.stringify(results)}`,
          results,
        );
        ...
        console.error("Google Calendar sync stream error:", error);
        send("error", {
          type: "error",
