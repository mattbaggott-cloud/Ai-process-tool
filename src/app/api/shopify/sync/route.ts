...
import { autoLinkByEmail } from "@/lib/shopify/identity-linker";
    ...
    async start(controller) {
      ...
      try {
        ...
        try {
          ...
          // Auto-link ecom customers to CRM contacts by email
          const linkResult = await autoLinkByEmail(supabase, orgId, user.id);
          if (linkResult.linked > 0) {
            await logSync(supabase, user.id, orgId, connector.id, "info",
              `Identity linking: ${linkResult.linked} new links, ${linkResult.already_linked} existing`);
          }
        } catch (postSyncError) {
