...
import type { DataConnector, HubSpotSyncDirection } from "@/lib/types/database";
...
export default function HubSpotConnectorCard({ connector, onRefresh, onConfigure }: Props) {
  ...
  const handleSync = async (direction: HubSpotSyncDirection) => {
    setSyncing(true);
    setSyncResults(null);
    setSyncSteps([]);
    setError(null);
  ...
  const formatStepLabel = (key: string) => {
    const labels: Record<string, string> = {
      ...
      companies_import: "Companies",
      deals_import: "Deals",
      contacts_export: "Contacts",
      companies_export: "Companies",
      deals_export: "Deals",
    };
    ...
    <div className={`data-connector-card ${isConnected ? "data-connector-card-connected" : ""} ${isError ? "data-connector-card-error" : ""} ${syncing ? "data-connector-card-syncing" : ""}`}>
      ...
      <div className="hubspot-card-header">
        ...
        <div className="hubspot-card-title-row">
          <span className="data-connector-name">HubSpot</span>
          {isConnected && (
      ...
      <div className="data-connector-body">
        <div className="data-connector-desc">
          {isConnected
            ...
            : "Sync contacts, companies, and deals from HubSpot CRM"
          }
        </div>
        ...
        {syncing && syncSteps.length === 0 && (
          <div className="hubspot-sync-progress">
            ...
            <span className="hubspot-sync-progress-label">Connecting to HubSpot...</span>
          </div>
        )}
      ...
      <div className="data-connector-footer">
        ...
        {isConnected && (
          <div className="hubspot-actions">
            ...
            <div className="hubspot-sync-dropdown-wrap">
              <button
              ...
              {showSyncOptions && (
                <div className="hubspot-sync-dropdown">
                  <button onClick={() => handleSync("import")}>
                    Import from HubSpot
                  </button>
                  <button onClick={() => handleSync("export")}>
                    Export to HubSpot
                  </button>
